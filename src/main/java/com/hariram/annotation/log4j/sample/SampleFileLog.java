 ...
 */
@Log4J(
		appenders={APPENDER_TYPE.FILE},
		pattern="%-5p %c{1}:%L - %m%n",
		layout=LAYOUT.PATTERN,
		level=LOG_LEVEL.INFO,
		file="/home/hariram/output1.log",
		maxFiles=3		
		)
public class SampleFileLog {
	public static Logger LOGGER = null;
	static {
		/*AnnotationProcessor processor = new Log4JAnnotationProcessor();
		processor.process(new SampleClass());*/
		AnnotationUtil.processAnnotation(new Log4JAnnotationProcessor(), new SampleFileLog());
	}
